#!/usr/bin/env bash
# Build the RaceBench workload's container image and pack it into a
# docker-archive tarball at workloads/racebench/images.tar. That file and
...
# scheduler itself is guest infrastructure (sched_ext BPF + scx-init in
# nix/podman-initrd.nix), and thread-fuzz is bind-mounted into the container by
# the guest, so neither is part of this image. The guest kernel must be built
# with sched_ext + BTF (see nix/guest-kernel.nix).
#
# Usage:  ./build.sh
#
# Requires a working `docker` daemon (or `podman` with a `docker` shim) and
# network access (the image build fetches Debian packages: gcc, g++, make).
...
$DOCKER save bedrock/racebench:latest -o images.tar
